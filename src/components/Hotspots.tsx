...
import { Flame, MapPin, TrendingUp, List, Map as MapIcon, Users, ChevronRight, Search, X, Plus, Clock, Navigation, Crown } from 'lucide-react';
...
interface HotspotsProps {
    ...
    onToggleFollow?: (placeName: string) => void;
    followedPlaces?: string[];
}

export const Hotspots: React.FC<HotspotsProps> = ({ onPlaceClick, onToggleFollow, followedPlaces = [] }) => {
    ...
    const initMap = () => {
        ...
        hotspots.forEach((spot) => {
            if (spot.coordinates) {
                ...
                const hasTrendingPlace = spot.places.some(p => p.trend === 'up');
                let pulseColor = "#3b82f6"; // Default Blue (Low/Stable)
                if (hasTrendingPlace) pulseColor = "#ef4444"; // Red (High/Rising)
                                ...
                                <div className="grid gap-3">
                                    {area.places.map((place, pIdx) => (
                                        <div
                                            ...
                                            className="flex items-center justify-between bg-slate-950/40 p-3 rounded-xl border border-slate-800 hover:border-brand-500/50 transition group cursor-pointer hover:bg-brand-900/20"
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <div className="font-bold text-base text-slate-200 group-hover:text-brand-400 transition flex items-center gap-2">
                                                        {place.name}
                                                        {/* Follow Button */}
                                                        {onToggleFollow && (
                                                            <div onClick={(e) => e.stopPropagation()}>
                                                                <FollowButton
                                                                    isFollowing={followedPlaces.includes(place.name)}
                                                                    onToggle={(e) => {
                                                                        e.stopPropagation();
