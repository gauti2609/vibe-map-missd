...
import { checkHandleAvailability } from '../services/userService';
...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
    ...
    // Check if user is already authenticated (but profile missing) - ONLY IN PROD
    React.useEffect(() => {
        if (!IS_DEV && auth.currentUser) {
            setStep(2);
        }
    }, []);
    ...
    const [manualHandleError, setManualHandleError] = useState('');
    ...
    const handleGenerate = async () => {
        ...
        // Filter out taken handles
        const availableNames: string[] = [];
        for (const name of names) {
            const isAvailable = await checkHandleAvailability(name);
            if (isAvailable) availableNames.push(name);
        }

        setSuggestions(availableNames.length > 0 ? availableNames : ["VibeMaster_X", "Neon_Soul_01"]); // Fallback
        setLoading(false);
    };
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-brand-900 via-slate-950 to-slate-950 p-4">
                <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl text-center space-y-6 animate-fade-in-up">
                    ...
                    <button
                        onClick={async () => {
                            // Real Firebase Login for Deployment AND Dev
                            try {
            ...
            <div className="w-full max-w-lg space-y-6">
                ...
                <div className="bg-slate-900/40 border border-white/5 p-5 rounded-2xl relative space-y-2">
                    ...
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            setManualName(val);
                            setSelectedName('');
                            setManualHandleError('');

                            if (val.length > 3) {
                                setIsCheckingHandle(true);
                                const isAvailable = await checkHandleAvailability(val);
                                setIsCheckingHandle(false);
                                if (!isAvailable) {
                                    setManualHandleError("Handle is already taken.");
                                }
                            }
                        }}
                    ...
                    />
                    {isCheckingHandle && <p className="text-slate-500 text-[10px] mt-1">Checking availability...</p>}
                    {manualHandleError && (
                        <p className="text-red-500 text-[10px] font-medium mt-1">{manualHandleError}</p>
                    )}
                ...
                <button
                    onClick={async () => {
                        ...
                        const interestsList = Object.values(selectedInterests).reduce((acc, val) => acc.concat(val), [] as string[]);


                        if (!auth.currentUser) return;

                        // Final Uniqueness Check (Fail-safe)
                        const isAvailable = await checkHandleAvailability(finalName);
                        if (!isAvailable) {
                            alert("This handle was just taken! Please choose another.");
                            return;
                        }

                        const newUser: User = {
                            id: auth.currentUser.uid,
                            displayName: auth.currentUser.displayName || finalName, // PRESERVE REAL NAME
                            email: auth.currentUser.email || '', // Persist Email
                            handle: finalName,
                            avatarUrl: auth.currentUser.photoURL || `https://ui-avatars.com/api/?name=${finalName}&background=7c3aed&color=fff`,
                            isConnected: true,
                            interests: interestsList,
                            trustScore: 80,
                            followersCount: 0,
                            followingCount: 0,
                            followers: [], // Initialize empty array
                            following: [], // Initialize empty array

                            vibes: interestsList, // Populate vibes with selected interests so stats work
                            isNewUser: true, // Flag to trigger Roots popup
                        };
                        try {
                    ...
                    disabled={!finalName || !!manualHandleError || isCheckingHandle}
